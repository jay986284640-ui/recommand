...
研究范围:**Hive 读侧适配器、字典 / 映射数据结构、LLM prompt 设计、增量与降级策略、CI 脱机能力**。研究边界 = `agent-platform/training-data-synonym/` 子包内部;不涉及 LP Agent 主体、不改 `data-pipeline/`、不改 `tabale_structer.sql`。
...
| 选项 | 优点 | 缺点 |
|------|------|------|
| (A) 全独立 Python 包 `training_data_synonym/` | Library-First(P-I)清晰;依赖单向;CI 隔离;可独立 release | 部分 dataclass(`LLMClient` 抽象)需复制 |
| (B) 复用 `data-pipeline.feature_extraction.training_data/` 子模块 | 复用 LLMClient / 字典加载 | 反向耦合到 data-pipeline;违反 Library-First |

**Decision**:**(A) 全独立**。

**Rationale**:Constitution Principle I("Library-First")明确要求子包自包含;`agent-platform/training-data-synonym/` 已是顶层兄弟目录,与 `data-pipeline/` 平级。少量重复(LLMClient 抽象)通过**接口契约**层面对齐,避免代码反向依赖。
...
| `agent-platform/training-data-synonym/scripts/*.py` | demo legacy | 保留为兼容薄壳,实际逻辑迁入 `training_data_synonym/` 包 |
| `agent-platform/training-data-synonym/configs/dim_dictionary.yaml` | 7 维 | **改为 8 维**(新增 `consumable_type`),版本 1.0 → 2.0 |
| `agent-platform/training-data-synonym/configs/brand_dictionary.yaml` | 60+ 品牌 | 保留 |
| `agent-platform/training-data-synonym/docs/ALIGNMENT_cib_o2o.md` | 已对齐 v2.x | 微更新:加 consumable_type 段落、Hive 数据源段落 |
